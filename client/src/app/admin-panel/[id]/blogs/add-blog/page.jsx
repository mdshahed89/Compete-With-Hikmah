  ...
  const addBlogHandler = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/blogs/admin`,
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 mb-10 justify-between">
        <h2 className="text-3xl font-semibold text-gray-800">Add Blog</h2>
        <Link
      ...
      <form onSubmit={addBlogHandler} className="space-y-3">
        ...
        <div className=" pt-5 ">
          ...
          >
            {loading ? <ButtonLoading /> : "Add Blog"}
          </motion.button>
        </div>
      </form>
    </motion.div>
