...
const Page = ({ params }) => {
  const { id, carouselId } = React.use(params);
  ...
  const fetchCarouselInfo = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/carousel/${carouselId}`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        setCarouselInfo({
          title: data?.carouselItem?.title || "",
          description: data?.carouselItem?.description || "",
          link: data?.carouselItem?.link || "",
          imageUrl: data?.carouselItem?.imageUrl || "",
        });
      } else {
        console.log(data.message || "Failed to fetch carousel!");
        router.push(`/admin-panel/${id}/carousels`);
      }
    } catch (error) {
  ...
  const updateCarouselHandler = async (e) => {
    e.preventDefault();
    setLoading(true);
    try {
      const response = await fetch(
      ...
      toast.error("Something went wrong! Please try again.");
      console.error("Error submitting update carousel:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 mb-10 justify-between">
        <h2 className="text-3xl font-semibold text-gray-800">Update Carousel</h2>
        <Link
      ...
      <form onSubmit={updateCarouselHandler} className="space-y-3">
        ...
        <div className=" pt-5 ">
          ...
          >
            {loading ? <ButtonLoading /> : "Update Carousel"}
          </motion.button>
        </div>
      </form>
    </motion.div>
