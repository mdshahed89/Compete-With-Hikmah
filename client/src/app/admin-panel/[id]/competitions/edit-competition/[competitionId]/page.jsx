...
const Page = ({ params }) => {
  ...
  const fetchCompetitionInfo = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/competition/${competitionId}`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        console.log(data.message || "Failed to fetch competition!");
        router.push(`/admin-panel/${id}/competitions`);
      }
    } catch (error) {
  ...
  const updateCompetitionHandler = async (e) => {
    e.preventDefault();
    setLoading(true);
    try {
      const response = await fetch(
      ...
      toast.error("Something went wrong! Please try again.");
      console.error("Error submitting update competition:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  console.log(competitionInfo.date);
  ...
  return (
    ...
    >
      ...
      <form onSubmit={updateCompetitionHandler} className="space-y-3">
        ...
        <div className=" mt-3 ">
          ...
          <input
            type="date"
            name="date"
            value={competitionInfo.date ? competitionInfo.date.slice(0, 10) : ''}
        ...
        <div className=" pt-5 ">
          ...
          >
            {loading ? <ButtonLoading /> : "Update Competition"}
          </motion.button>
        </div>
      </form>
    </motion.div>
